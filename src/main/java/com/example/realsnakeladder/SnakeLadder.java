...
import static com.example.realsnakeladder.GameBoard.tileSize;

public class SnakeLadder extends Application {
    public final int titleSize = 40;
    ...
    public Pane createContent(){
        Pane root = new Pane();
        root.setPrefSize(width*titleSize, height*titleSize+80);
        root.getChildren().addAll(tileGroup);
        for(int i =0; i<height; i++){
            for(int j =0; j<width; j++){
                Tile tile = new Tile(titleSize,titleSize);
        ...
        Image img = new Image("C:\\Users\\Dabloo Gupta\\IdeaProjects\\RealSnakeLadder\\src\\SnakeLadderBoard12 november.jpg");
        ImageView boardImage = new ImageView();
        boardImage.setImage(img);
        boardImage.setFitWidth(titleSize*width);
        boardImage.setFitHeight(titleSize*height);

        tileGroup.getChildren().addAll(boardImage,playerOneButton,gameButton,playerTwoButton,randResult,
                playerOne.getGamePiece(),playerTwo.getGamePiece());//when we add player button then sohow it
        return root;
    }
